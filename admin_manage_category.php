...
$totalPage = ceil($categoryModel->getTotalCategories() / $perPage);
/* Pagination - End */

$categoriesList = $categoryModel->getCategoriesByPage($page, $perPage);
?>
...
<head>
  ...
  <title>Quản lý danh mục - P3 Pet Shop</title>
...
<body>
  ...
  <section class="admin-sidebar">
    ...
    <ul class="admin-side-bar-list">
      ...
      <li class="admin-side-bar-item"><a class="admin-side-bar-link active" href="admin_manage_category.php"><i class="fa-regular fa-folder-open"></i> Quản lý danh mục</a></li>
  ...
  <section class="admin-content">
    <header class="header">
      <nav class="nav-bar">
        <ul class="nav-list">
          <li class="nav-item">
            <form class="serch-box" action="admin_serch_category.php?key=<?php if (isset($key)) {
                                                                            echo $key;
                                                                          } ?>">
    ...
    <main>
      <div class="table-wrap">
        <div class="table-bar mb-4">
          <a class="btn btn-primary" href="admin_add_category.php"><i class="fa-solid fa-file-circle-plus"></i> Thêm danh mục</a>
          <section class="d-flex align-items-center">
            <span class="d-inline">Số hàng: </span>
            <form method="post" action="admin_manage_category.php" class="form-per-page d-inline-block ms-2">
        ...
        <table class="table table-hover table-dark">
          <thead>
            <tr>
              <th><i class="fa-solid fa-list"></i> Tên danh mục</th>
              <th><i class="fa-solid fa-table-list"></i> Danh mục cha</th>
          ...
          <tbody>
            ...
            foreach ($categoriesList as $category) {
              if ($category['parent_id'] == 0) {
                $parentCategory = 'Không';
              } else {
                $parentCategory = $categoryModel->getCategoryByID($category['parent_id'])['category_name'];
              }
            ?>
              <tr>
                <td><span class="table-content"><?php echo $category['category_name']; ?></span></td>
                <td><span class="table-content"><?php echo $parentCategory; ?></span></td>
                <td>
                  <a class="btn btn-warning mb-2 mt-2" href="admin_edit_category.php?id=<?php echo $category['id']; ?>"><i class="fa-solid fa-pencil"></i> Sửa thông tin</a>
                  <form action="admin_delete_category.php" method="post" onsubmit="return confirm('Khi xóa danh mục `<?php echo $category['category_name']; ?>` các danh mục con và thú cưng bên trong của nó cũng sẽ bị xóa. Bạn có chắn chắc muốn xóa ?')">
                    <input type="hidden" name="deleteId" value="<?php echo $category['id'] ?>">
                    <button class="btn btn btn-danger mb-2 mt-2" type="submit"><i class="fa-solid fa-trash"></i> Xóa danh mục</button>
                  </form>
                </td>
              </tr>
            <?php
        ...
        <nav aria-label="Page navigation" class="mt-4">
          <ul class="pagination justify-content-center">
            ...
            ?>
              <li class="page-item">
                <a class="page-link" href="admin_manage_category.php"><i class="fa-solid fa-circle-left"></i></a>
              </li>
              <li class="page-item">
                <a class="page-link" href="admin_manage_category.php?page=<?php echo $page - 1; ?>"><i class="fa-regular fa-circle-left"></i></a>
              </li>
            <?php
            }
            ?>
            ...
            for ($pageItem = 1; $pageItem <= $totalPage; $pageItem++) {
              if ($pageItem == $page) {
            ?>
                <li class="page-item active"><a class="page-link" href="admin_manage_category.php?page=<?php echo $pageItem; ?>"><?php echo $pageItem; ?></a></li>
              <?php
              } else {
              ?>
                <li class="page-item" <?php if ($pageItem > $getAdd || $pageItem < $getMinus) {
                                        echo "style='display:none;position:absolute'";
                                      } ?>><a class="page-link" href="admin_manage_category.php?page=<?php echo $pageItem; ?>"><?php echo $pageItem; ?></a></li>
            <?php
              }
            }
            ?>
            ...
            ?>
              <li class="page-item">
                <a class="page-link" href="admin_manage_category.php?page=<?php echo $page + 1; ?>"><i class="fa-regular fa-circle-right"></i></a>
              </li>
              <li class="page-item">
                <a class="page-link" href="admin_manage_category.php?page=<?php echo $totalPage; ?>"><i class="fa-solid fa-circle-right"></i></a>
              </li>
            <?php
            }
            ?>
